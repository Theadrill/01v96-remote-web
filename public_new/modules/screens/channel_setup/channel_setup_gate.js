/**
 * =========================================================================================
 * SCREEN CONTROLLER: Channel Setup - Gate Panel (channel_setup_gate.js)
 * =========================================================================================
 * Responsabilidade:
 * - Controlador do painel de Gate dentro da aba de Dinâmica na Central de Edição do Canal.
 * - Gerencia disponibilidade de Gate (disponível apenas para Inputs 0-31 na Yamaha 01V96).
 * - Sincroniza parâmetros de Threshold, Range, Attack, Hold e Decay via WebSocket.
 *
 * Plano de Transição (Fase 7):
 * - Este controlador passará a alimentar o componente visual puro `modules/components/gate.js`,
 *   repassando valores de estado e escutando eventos de alteração de controles.
 * =========================================================================================
 */

function renderGate(container, ch) {
    // 01V96: Gate só existe nos canais de entrada (0-31)
    if (ch >= 36) {
        ...
        box.innerHTML = `
            <div class="dyn-header">
                <div class="dyn-label">GATE (N/A)</div>
            </div>
            <div style="padding:40px 20px; text-align:center; color:#666; font-size:12px; font-weight:bold;">
                O GATE NÃO ESTÁ DISPONÍVEL PARA OS CANAIS DE SAÍDA NA 01V96.
            </div>
        `;
    ...
    const HOLD_STEPS = [
        ...
        '0.43ms', '0.45ms', '0.47ms', '0.49ms', '0.51ms', '0.53ms', '0.55ms', '0.57ms', '0.59ms', '0.61ms', '0.63ms', '0.65ms', '0.70ms', '0.75ms', '0.80ms', '0.85ms', '0.90ms', '0.94ms', '1.02ms',
    ...
    const defaultState = { on: false, thresh: -260, range: -60, attack: 0, hold: 20, decay: 50 };
    const targetState = getChannelStateById(ch);
    const savedState = (targetState && targetState.gate) || {};
...
function updateGateFromSocket(ch, key, value) {
    ...
    if (!targetState.gate) targetState.gate = {};
    ...
    if (internalKey) targetState.gate[internalKey] = (key === 'kGateOn' ? !!value : value);

    const btn = document.getElementById('gateOn');
    if (btn) btn.classList.toggle('active', !!targetState.gate.on);
