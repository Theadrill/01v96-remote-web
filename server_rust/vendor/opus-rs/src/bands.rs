...
use crate::rate::{bits2pulses, get_pulses, pulses2bits, BITRES};
...
pub fn haar1(x: &mut [f32], n0: usize, stride: usize) {
    ...
    unsafe {
        if stride == 1 && n0 >= 16 && false && is_x86_feature_detected!("avx") {
            haar1_avx(x, n0);
            return;
        }
    }
...
pub fn compute_band_energies(
    ...
    #[cfg(target_arch = "x86_64")]
    let use_avx2 = false && std::arch::is_x86_feature_detected!("avx2");
...
pub fn renormalise_vector(x: &mut [f32], n: usize, gain: f32) {
    ...
    #[cfg(target_arch = "x86_64")]
    unsafe {
        if n >= 16 && false && std::arch::is_x86_feature_detected!("avx2") {
