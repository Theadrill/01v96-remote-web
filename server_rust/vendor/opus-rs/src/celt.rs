use crate::bands::{
    compute_band_energies, denormalise_bands, haar1, log2amp, normalise_bands, quant_all_bands,
    spreading_decision, SPREAD_NONE, SPREAD_NORMAL,
};
...
use crate::rate::{clt_compute_allocation, BITRES};
...
fn sum_abs(x: &[f32]) -> f32 {
    ...
    unsafe {
        if false && std::arch::is_x86_feature_detected!("avx") {
...
fn l1_metric(tmp: &[f32], n: usize, lm: i32, bias: f32) -> f32 {
    #[cfg(target_arch = "x86_64")]
    unsafe {
        if n >= 16 && false && std::arch::is_x86_feature_detected!("avx") {
