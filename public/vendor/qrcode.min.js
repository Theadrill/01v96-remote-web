/**
 * QRCode.js - Standard standalone QR Code generator
 * License: MIT
 */
var QRCode;
(function () {
	...
	function _getUTF8Length(sText) {
		var encodeURIlength = encodeURI(sText).match(/%[89ABab]/g);
		return sText.length + (encodeURIlength ? encodeURIlength.length : 0);
	}

	function _getTypeNumber(sText, correctLevel) {
		var nType = 1;
	...
	QRCodeModel.prototype = {
		...
		mapData: function (data, maskPattern) {
			...
			var bitIndex = 7;
			var byteIndex = 0;
			var maskFunction = QRUtil.getMaskFunction(maskPattern);
				...
				while (true) {
					for (var c = 0; c < 2; c++) {
						if (this.modules[row][col - c] == null) {
							...
							var mask = maskFunction(row, col - c);
							if (mask) dark = !dark;
	...
	QRCode.prototype.makeCode = function (sText) {
		...
		var typeNumber = _getTypeNumber(sText, this._htOption.correctLevel);
		var qrcode = new QRCodeModel(typeNumber, this._htOption.correctLevel);
		qrcode.addData(sText);
		qrcode.make();
		...
		for (var row = 0; row < nCount; row++) {
			for (var col = 0; col < nCount; col++) {
				if (qrcode.isDark(row, col)) {
					ctx.fillRect(Math.round(col * nWidth), Math.round(row * nHeight), Math.ceil(nWidth), Math.ceil(nHeight));
				}
			}
		}
