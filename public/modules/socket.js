...
// Re-renderiza o modal de Insert se estiver aberto para o canal informado
function rerenderOpenInsertModal(chIdx) {
    const overlay = document.getElementById('insertModalOverlay');
    if (!overlay || overlay.style.display !== 'flex') return;
    const openCh = window._insertModalChannel;
    if ((chIdx === undefined || chIdx === null || chIdx === openCh) && typeof window.openInsertModal === 'function') {
        window.openInsertModal(openCh);
    }
}
...
socket.on('fxInputsUpdate', (data) => {
    ...
    for (const [key, val] of Object.entries(data)) {
        ...
        // Sincroniza o INSERT IN do canal alvo do FX:
        // Se a fonte deste FX input aponta para o insert de um canal (valor 13..44 => INS CH 1..32),
        // o INSERT IN daquele canal recebe o retorno correspondente do FX (121+slot*8+lr).
        const srcCh = Math.round(Number(val));
        const slot = Math.floor(port / 2);
        const lr = port % 2;
        const fxReturnValue = 121 + slot * 8 + lr;

        if (srcCh >= 13 && srcCh <= 44) {
            const chIdx = srcCh - 13; // 0..31
            const chData = getChannelStateById(chIdx);
            if (chData && chData.insert) {
                chData.insert.patch_in = fxReturnValue;
                rerenderOpenInsertModal(chIdx);
            }
        } else {
            // Fonte removida/trocada: limpa o INSERT IN que apontava para este retorno FX
            let clearedAny = false;
            for (let chIdx = 0; chIdx < 32; chIdx++) {
                const st = window.channelStates && window.channelStates[chIdx];
                if (st && st.insert && Math.round(st.insert.patch_in) === fxReturnValue) {
                    st.insert.patch_in = 0;
                    clearedAny = true;
                }
            }
            if (clearedAny) rerenderOpenInsertModal();
        }
    }
    if (activeConfigTab === 'etc'
