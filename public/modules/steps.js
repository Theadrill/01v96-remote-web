...
async function loadMeterSteps() {
    try {
        // Adicionado cache-buster (?t=...) para garantir que o F5 carregue as alterações do JSON
        const response = await fetch('steps.json?t=' + Date.now());
        meterCalibration = await response.json();
        console.log("✅ [METERS] Calibração de steps carregada:", meterCalibration);
    } catch (e) {
...
window.calibrateStep = function(step, isMaster = false) {
    ...
    const dbValue = meterCalibration[step];
